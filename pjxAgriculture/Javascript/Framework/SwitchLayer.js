              ...
              "esri/symbols/Font"], function (GraphicsLayer, Point, Polyline, Polygon, PictureMarkerSymbol, Color, Graphic, SimpleLineSymbol, SimpleFillSymbol, TextSymbol, Font) {
                  ...
                  for (var i = 0; i < data.features.length; i++) {
                      if (data.features[i].properties.name == ssqy) {
                          ...
                          var blockCityNumber = getCityParam(sssy, "县市区名称",ssqy);
                          var showLabel = ssqy + "   " + blockCityNumber;
                          var attr = { "name": ssqy, "number": blockCityNumber };
                  ...
                  map.addLayer(blockTextLayer);
                  //  alert(2)
                  $("#total").text(total);
                  var blockTextClick = blockTextLayer.on("mouse-over", function (e) {
                  ...
                  var blockTextClick = blockTextLayer.on("mouse-out", function (e) {
                      //get the associated node info when the graphic is clicked
                      $("#typeinfo").hide();
